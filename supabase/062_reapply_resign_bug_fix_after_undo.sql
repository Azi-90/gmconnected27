...
-- "Undo Last Action" was clicked, which restores the ENTIRE players and
-- pending_contract_extensions tables to whatever they looked like right
-- before the most recent snapshotted action -- in this case, Kindel's
-- original (buggy) re-sign from before 060 ran. That's a full-table
-- restore, not a per-player undo, so it dragged Crosby/Malkin/Karlsson/
-- Mercer back to their pre-060 broken contracts and wiped every pending
-- extension row 060 had queued (Kindel's own row happened to end up
-- correct purely because his snapshot was taken before HIS bug applied).
-- This re-applies the exact same correction from 060 -- safe to run even
-- if some rows already happen to be right.
